...
public class DateUtil {
    ...
    public static Date startOfDate(LocalDate localDate) {
        return DateUtil.toDate(localDate.atStartOfDay());
    }

    public static Date endOfDate(LocalDate localDate) {
        return DateUtil.toDate(localDate.atTime(LocalTime.MAX));
    }
    ...
    public static Date countDate(Byte limitType) {
        LocalDate now = LocalDate.now();
        LocalDate countDate;
        if (limitType == 1) {
            countDate = now;
        } else if (limitType == 2) {
            DayOfWeek nowDayOfWeek = now.getDayOfWeek();  //获取当前星期几
            if (!nowDayOfWeek.equals(DayOfWeek.MONDAY)) {  //如果不等于星期一
                long backDays = nowDayOfWeek.getValue() - 1;  //回退到周一的天数
                countDate = now.minusDays(backDays);
            } else {
                countDate = now;
            }
        } else if (limitType == 3) {
            countDate = now.with(TemporalAdjusters.firstDayOfMonth());
        } else if (limitType == 4) {
            countDate = LocalDate.of(1998, 1, 1);  //这个系统201几年才开始用
        } else {
            return null;
        }
        return DateUtil.toDate(countDate);
    }
}
